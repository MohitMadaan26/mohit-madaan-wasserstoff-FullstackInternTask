  ...
  reducers: {
    ADD_TO_CART: (state, action) => {
      ...
      if (productIndex >= 0) {
        ...
        // Item doesnot already exist in the cart
    ...
    DECREASE_CART: (state, action) => {
      ...
      if (state.cartItems[productIndex].cartQuantity > 1) {
        state.cartItems[productIndex].cartQuantity -= 1;
        toast.info(`${action.payload.name} decrease to 1`, {
          position: "top-left",
        });
...
export const selectCartTotalQuantity = (state) =>
  state.cartReducer.cartTotalQuantity;
export const selectCartTotalAmount = (state) =>
  state.cartReducer.cartTotalAmount;
export const selectPreviousURL = (state) => state.cartReducer.previousURL;
